...
def run_single_pair(person_image_path, cloth_image_path, output_path, config_path, ckpt_path, ckpt_elbm_path, device, H=512, W=512):
    # Prepare temp dataset structure
    temp_dir = os.path.dirname(output_path)
    os.makedirs(temp_dir, exist_ok=True)
    test_id = "test_001"
    temp_dataset_dir = os.path.join(temp_dir, "temp_dataset")
    os.makedirs(os.path.join(temp_dataset_dir, "test", "image"), exist_ok=True)
    os.makedirs(os.path.join(temp_dataset_dir, "test", "cloth"), exist_ok=True)
    os.makedirs(os.path.join(temp_dataset_dir, "test", "cloth-mask"), exist_ok=True)
    os.makedirs(os.path.join(temp_dataset_dir, "test", "image-parse-v3"), exist_ok=True)
    os.makedirs(os.path.join(temp_dataset_dir, "test", "openpose_img"), exist_ok=True)
    os.makedirs(os.path.join(temp_dataset_dir, "test", "openpose_json"), exist_ok=True)
    os.makedirs(os.path.join(temp_dataset_dir, "test", "train_paired"), exist_ok=True)
    os.makedirs(os.path.join(temp_dataset_dir, "test", "train_paired", "mask"), exist_ok=True)
    os.makedirs(os.path.join(temp_dataset_dir, "test", "train_paired", "seg_preds"), exist_ok=True)
    os.makedirs(os.path.join(temp_dataset_dir, "test", "train_paired", "warped"), exist_ok=True)
    shutil.copy2(person_image_path, os.path.join(temp_dataset_dir, "test", "image", f"{test_id}.jpg"))
    shutil.copy2(cloth_image_path, os.path.join(temp_dataset_dir, "test", "cloth", f"{test_id}.jpg"))
    # Create a simple cloth mask (white rectangle)
    cloth_img = Image.open(cloth_image_path)
    mask_img = Image.new('L', cloth_img.size, 255)
    mask_img.save(os.path.join(temp_dataset_dir, "test", "cloth-mask", f"{test_id}.jpg"))
    ...
    if torch.cuda.is_available():
        model = model.cuda()
    model.eval()
    dataset = CPDataset(temp_dataset_dir, H, mode='test', unpaired=True)
...
def main():
    ...
    parser.add_argument('--person_image', type=str, default=None, help='Path to person image for single-pair inference')
    parser.add_argument('--cloth_image', type=str, default=None, help='Path to clothing image for single-pair inference')
    parser.add_argument('--output', type=str, default=None, help='Path to save the output image for single-pair inference')

    opt = parser.parse_args()

    seed_everything(opt.seed)

    if opt.person_image and opt.cloth_image and opt.output:
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        run_single_pair(opt.person_image, opt.cloth_image, opt.output, opt.config, opt.ckpt, opt.ckpt_elbm_path, device, H=opt.H, W=opt.W)
        return
